...
input_list <- read.table("genes_sets.txt", sep = "\t", header = T, check.names = F, quote = "")
# input_list <- input_list$genes
list <- split(
  input_list$GeneSymbol,
  list(
    input_list$set,
    input_list$direction
  ),
  drop = TRUE          # per rimuovere i gruppi vuoti
)
